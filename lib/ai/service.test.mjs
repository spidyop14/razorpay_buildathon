...
test("13. demo query 'Coding laptop under ₹50K' matches CodeLite 14", () => {
  const intent = parseBuyerRequest("Coding laptop under ₹50,000 with 16GB RAM");
  assert.equal(intent.category, "laptop");
  assert.equal(intent.maxBudget, 50000);
  assert.equal(intent.requirements?.ram, "16GB");
  const matches = searchCatalog(intent);
  assert.ok(matches.length > 0);
  assert.equal(matches[0].product.id, "lap-003");
  assert.equal(matches[0].product.name, "CodeLite 14");
  assert.equal(matches[0].product.price, 47999);
});
...
test("16. missing requirements object still validates", () => {
  const { requirements: _omit, ...rest } = valid;
  const intent = validateBuyerIntent(rest);
  assert.equal(intent?.category, "laptop");
  assert.deepEqual(intent?.requirements, {});
});

test("17. empty requirements object still validates", () => {
  const intent = validateBuyerIntent({ ...valid, requirements: {} });
  assert.equal(intent?.maxBudget, 60000);
  assert.deepEqual(intent?.requirements, {});
});

test("18. Gemini fallback still returns valid catalog products", async () => {
  const provider = {
    generateBuyerIntent: async () => {
      throw new AIError("AI_PROVIDER_ERROR", "Gemini unavailable");
    },
    generateRecommendationExplanation: async () => "",
  };
  const gaming = await understandBuyerRequest("Gaming setup under ₹80K", [], provider);
  assert.equal(gaming.mode, "fallback");
  const gamingIds = searchCatalog(gaming.intent).map((x) => x.product.id);
  assert.ok(gamingIds.includes("game-001"));
  assert.ok(gamingIds.includes("game-002"));

  const coding = await understandBuyerRequest("Coding laptop under ₹60K", [], provider);
  const codingIds = searchCatalog(coding.intent).map((x) => x.product.id);
  assert.ok(codingIds.includes("lap-001"));
  assert.ok(codingIds.includes("lap-002"));

  const travel = await understandBuyerRequest("Travel headphones under ₹5K", [], provider);
  const travelIds = searchCatalog(travel.intent).map((x) => x.product.id);
  assert.ok(travelIds.includes("head-001"));
  assert.ok(travelIds.includes("head-002"));
});

test("19. mocked Gemini gaming intent still matches catalog products", async () => {
  const provider = {
    generateBuyerIntent: async () =>
      validateBuyerIntent({
        ...
        minBudget: null,
        maxBudget: 80000,
        requirements: { ram: null, portability: null },
        preferences: ["value"],
        sortPreference: "value",
        confidence: 0.96,
        clarificationNeeded: false,
  ...
  const r = await understandBuyerRequest("Gaming setup under ₹80K", [], provider);
  assert.equal(r.mode, "ai");
  const ids = searchCatalog(r.intent).map((x) => x.product.id);
